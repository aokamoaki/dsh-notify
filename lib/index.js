...
/**
 * Browser-trust fence for the plugin routes: the Host must be ours, the
 * request must not carry a cross-site marker, and any Origin must match.
 *
 * The Host pin is what makes this a fence. `sec-fetch-site` alone is not
 * enough: a DNS-rebinding page resolves its own hostname to 127.0.0.1 and
 * issues what the browser labels a same-origin request, so the marker says
 * `same-origin` (or is absent) while the page is hostile. The Host header
 * still carries the attacker's name, which is why it is checked first.
 *
 * Behaviourally identical to the /api gateway's own fence — these routes are
 * dispatched by longest-prefix match ahead of that gateway and never see it.
 */
