 ...
 *   - turn/end(completed) + agent idle  -> "done" notification (800ms debounce)
 *   - turn/end(error)                   -> "error" notification
...
export function apply(ctx, config = {}) {
  ...
  ctx.on('session/event', (session, event) => {
    ...
    if (event.type === 'turn/start') { turnStart.set(session.id, event.time); return; }
    ...
    if (reason === 'completed' && agentPhaseKind === 'idle') {
      // Debounce: with queued messages a new turn may start right after; only
      // notify when the agent stays idle.
      setTimeout(() => {
        if (agentPhaseKind === 'idle') notify(session.id, '「' + (titleOf(session) || tr('conv')) + '」', tr('done') + suffix, 'done');
      }, 800);
    } else if (reason === 'error') {
      notify(session.id, '「' + (titleOf(session) || tr('conv')) + '」', tr('failed') + suffix, 'error');
    }
    // aborted / blocked / max-tokens / interrupted: no notification.
  });
